...
def do_one(dest_addr, timeout=10, psize=64):
    """
...
def quiet_ping(dest_addr, timeout = 20, count = 10, psize = 64):
    """
...
def send_mail(msgtxt):
    tolist = ["*@qq.com","*@qq.com","*@qq.com"] #多个收件人
    msg = MIMEMultipart()   
    msg['From'] = '*'  #发件人
    msg['To'] = "*,*,*"#收件人
    msg['Subject'] = 'chenk_ping'  
    txt = MIMEText("网络异常 \r" + msgtxt)
    msg.attach(txt)
    smtp = smtplib.SMTP()
    smtp.connect('smtp.qq.com:25')
    smtp.login('*', '*')#用户名密码
    smtp.sendmail('*',tolist, msg.as_string())#发件人地址
    smtp.quit()
    print '发送成功'
def check_ping_result(quiet):
...
if __name__ == '__main__':
    check_ping_result("www.github.com") 
